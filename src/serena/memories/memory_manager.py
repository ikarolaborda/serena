...
class MemoryManager:
    ...
    def __init__(
        ...
        # Cross-process safety is provided by the per-file lock in `_lock_path_for`.
    ...
    def move_memory(self, old_name: str, new_name: str, is_tool_context: bool) -> str:
        ...
        with self._instance_lock, cross_process_lock(self._lock_path_for(old_path)), cross_process_lock(self._lock_path_for(new_path)):
    ...
    def search_memories(
        ...
        *,
    ...
    ) -> list["MemoryManager.MemoryMatch"]:
        """
        ...
        results: list[MemoryManager.MemoryMatch] = []
        seen_memories = 0
