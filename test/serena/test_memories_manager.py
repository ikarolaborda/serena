...
# --- Fork additions: frontmatter parsing, tag-filtered search, metadata ---


@pytest.fixture
def memories_manager(tmp_path) -> MemoryManager:
    serena_data_folder = tmp_path / ".serena"
    serena_data_folder.mkdir()
    return MemoryManager(serena_data_folder=str(serena_data_folder))


class TestFrontmatterParsing:
    def test_no_frontmatter_returns_empty_metadata(self) -> None:
        metadata, body = MemoryManager.parse_frontmatter("# Some title\n\nbody text\n")
        assert metadata == {}
    ...
    def test_well_formed_frontmatter_extracts_supported_fields(self) -> None:
        ...
        metadata, body = MemoryManager.parse_frontmatter(raw)
        assert metadata == {
            "tags": ["bug", "auth"],
            "type": "incident",
            "description": "post-mortem of session leak",
    ...
    def test_csv_tags_form_is_accepted(self) -> None:
        raw = "---\ntags: bug, auth, perf\n---\nbody\n"
        metadata, _ = MemoryManager.parse_frontmatter(raw)
    ...
    def test_serialize_then_parse_roundtrip(self) -> None:
        block = MemoryManager.serialize_frontmatter({"tags": ["x", "y"], "type": "decision"})
        parsed, body = MemoryManager.parse_frontmatter(block + "body\n")
        assert parsed == {"tags": ["x", "y"], "type": "decision"}
        assert body == "body\n"


class TestSearchMemories:
    def _write(self, mgr: MemoryManager, name: str, body: str) -> None:
        mgr.save_memory(name, body, is_tool_context=False)

    def test_substring_search_returns_matches(self, memories_manager: MemoryManager) -> None:
    ...
    def test_regex_mode(self, memories_manager: MemoryManager) -> None:
    ...
    def test_tags_filter_skips_untagged(self, memories_manager: MemoryManager) -> None:
        self._write(
            memories_manager,
            "tagged",
            MemoryManager.serialize_frontmatter({"tags": ["bug"]}) + "Has the keyword foo.\n",
        )
    ...
    def test_caps_are_respected(self, memories_manager: MemoryManager) -> None:
        body = "\n".join(["match here"] * 50) + "\n"
        self._write(memories_manager, "long", body)
        results = memories_manager.search_memories("match", max_matches_per_memory=3)
        assert len(results) == 3

    def test_snippet_truncation(self, memories_manager: MemoryManager) -> None:
    ...
    def test_invalid_regex_raises(self, memories_manager: MemoryManager) -> None:
        self._write(memories_manager, "any", "content\n")
        with pytest.raises(Exception):
            memories_manager.search_memories("[unclosed", mode="regex")

    def test_frontmatter_body_not_polluted_by_match_count(self, memories_manager: MemoryManager) -> None:
        # The frontmatter delimiter "---" must not be matched when searching for "---"
        self._write(
            memories_manager,
            "fm",
            MemoryManager.serialize_frontmatter({"tags": ["a"]}) + "the body has the word frontier\n",
        )
        results = memories_manager.search_memories("---")
        assert results == []

    def test_write_then_search_e2e(self, memories_manager: MemoryManager) -> None:
        """End-to-end: tags written via frontmatter are queryable via tag filter
        and surface in read_memory_metadata."""
        body = MemoryManager.serialize_frontmatter({"tags": ["bug", "auth"], "type": "incident"}) + (
